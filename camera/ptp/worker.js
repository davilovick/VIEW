...
process.on("message", function(msg) {
  ...
  if (msg.type == "camera" && camera) {
    ...
    if (msg.get == "settings") {
      console.log(
        "WORKER: called getConfig in",
        new Date() / 1000 - msg.time,
        "seconds"
      );
    ...
    fs.writeFile(thumbnailPath + "/count.txt", indexStr, function() {
      ...
      image.downsizeJpeg(
        ...
        function(err, jpgBuf) {
          if (!err && jpgBuf) {
            fs.writeFile(
              thumbnailFileFromIndex(index, cameraIndex),
              jpgBuf,
              function() {
                console.log(
                  "WORKER: completed saveThumbnail",
                  index,
                  "in",
                  new Date() / 1000 - thumbnailStartTime,
                  "seconds"
                );
              }
            );
...
function processRawPath(path, options, info, callback) {
  ...
  image.getJpegFromRawFile(path, null, function(err, jpg) {
    ...
    if (options.saveRaw) {
      ...
      execFile("/bin/cp", ["--no-target-directory", path, dest], {}, function(
        err,
        stdout,
        stderr
      ) {
        if (err || stderr) {
          ...
          if (stderr.indexOf("No space left on device") !== -1) {
            sendEvent(
              "saveErrorCardFull",
              "Error saving RAW file " + dest + "\nNo space left on SD card."
            );
          } else {
            sendEvent(
              "saveError",
              "Error saving RAW file " +
                dest +
                "\nError code: " +
                err +
                ", message: " +
      ...
      if (options.index || options.index === 0) {
        ...
        var desc =
          name +
          " created with the Timelapse+ VIEW\nImage #" +
          options.index +
          "\nBase Exposure: " +
          options.exposureCompensation;
        if (options.exposureCompensation != null)
          image.writeXMP(dest, options.exposureCompensation, desc, name);
      }
    } else {
      sdWriting = false;
      fs.unlink(path);
    ...
    if (options.index || options.index === 0) {
      ...
      image.downsizeJpeg(jpg, size, null, function(err, lowResJpg) {
        ...
        image.exposureValue(img, function(err, ev, histogram) {
          ...
          console.log(
            "WORKER: ev:",
            ev,
            " (compensation: " + options.exposureCompensation + ")"
          );
      ...
      sendEvent(
        "status",
        "photo saved to " + dest.replace("/media", "SD card: ")
      );
      if (callback) {
        callback(err, {
          file: dest
...
function capture(options, callback) {
  ...
  camera.takePicture(captureOptions, function(err, photo, info) {
    console.log(
      "WORKER: running camera.takePicture() -> callback(). File:",
      info
    );
    cameraBusy = false;
    if (!err && photo) {
      ...
      if (options.thumbnail && supports.thumbnail) {
        ...
        if (options.calculateEv) {
          ...
          image.downsizeJpeg(photo, size, null, function(err, lowResJpg) {
            ...
            if (!err && lowResJpg) {
              console.log(
                "WORKER: running luminance calc on low-res preview...",
                lowResJpg.length,
                "bytes"
              );
              img = lowResJpg;
              ...
              console.log(
                "WORKER: running luminance calc on full-res preview..."
              );
              img = photo;
            }
            ...
            image.exposureValue(img, function(err, ev, histogram) {
              console.log(
                "WORKER: adjusting ev by ",
                options.exposureCompensation
              );
              ev = ev + options.exposureCompensation;
              var processingTime = new Date() / 1000 - startTime;
              console.log(
                "WORKER: luminance calc complete. ev:",
                ev,
                "Processed in ",
                processingTime,
                "seconds"
              ...
              if (callback)
                callback(err, {
                  ...
                  histogram: histogram,
                  file: info,
                  thumbnailPath: thumbnailFileFromIndex(
                    options.index,
                    options.cameraIndex
                  )
                });
          ...
          if (callback)
            callback(err, {
              ...
              thumbnailPath:
                options.index != null
                  ? thumbnailFileFromIndex(options.index, options.cameraIndex)
                  : null
            });
        }
        var size = {
      ...
      if (errCount > 5) {
        ...
        console.log(
          "WORKER: Error during capture:",
          err,
          "(retrying " + errCount + " of 5)"
        );
        setTimeout(function() {
          capture(options, callback);
        });
      ...
      function(err1, event, path) {
        if (!err1 && event == "file_added" && path) {
                ...
                if (thumbnail) {
                  image.getJpegBuffer(tmp, function(err3, jpg) {
                    if (!err3 && jpg) {
                      ...
                      if (thumbnailPath) {
                        fs.readFile(thumbnailPath + "/count.txt", function(
                          err,
                          data
                        ) {
                          var count = 0;
                      ...
                      image.exposureValue(jpg, function(err, ev, histogram) {
                        ...
                        sendEvent(
                          "status",
                          "photo ev: " + Math.round(ev * 100) / 100
                        );
                        sendEvent("ev", ev);
                        sendEvent("histogram", histogram);
          ...
          if (seconds - startSeconds < timeoutSeconds) {
            ...
            console.log(
              "WORKER: tethered capture timed out at ",
...
function set(item, value, callback) {
  // item can be 'iso', 'aperture', 'shutter', etc
  console.log(
    "WORKER: setting " + item + " to " + value + " (" + typeof value + ")"
  );
    ...
    if (list) {
      ...
      for (var i = 0; i < list.length; i++) {
        if (list[i].cameraName == value || list[i].name == value) {
          ...
          camera.setConfigValue(item, list[i].cameraName || value, function(
            err
          ) {
            if (err) sendEvent("error", err);
            if (callback) callback(err);
      ...
      console.log(
        "WORKER: error",
        "item not found in list: " + item + " = [" + value + "] (trying anyway)"
      );
      camera.setConfigValue(item, value, function(err) {
        if (err) sendEvent("error", err);
        if (callback) callback(err);
...
function getConfig(noEvent, cached, cb) {
  ...
  camera.getConfig(function(er, data) {
    ...
    if (data && data.main && data.main.children) {
      ...
      for (var i in LISTS.paramMap) {
        ...
        for (m in maps) {
          ...
          if (section == null && data[item] && data[item].available) {
            ...
            if (
              list &&
              (handle == "shutter" || handle == "iso" || handle == "aperture")
            ) {
              var listHalfs = mapCameraList(
                handle + "Halfs",
                data[item].available
              );
            ...
            name = item;
            if (detail)
              console.log(name + " = " + value + " (" + detail.name + ")");
            break;
          } else {
            try {
              ...
              if (
                item == "shutterspeed" &&
                data.status &&
                data.status.children.manufacturer.value == "Sony Corporation"
              ) {
                console.log(
                  "WORKER: manually adding shutter speed list (" +
                    (halfsUsed ? "halfs" : "thirds") +
                    ")",
      ...
      console.log(
        "WORKER: Mapped settings in ",
        new Date() / 1000 - getConfigStartTime,
        "seconds"
      );
